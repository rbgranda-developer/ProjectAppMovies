...
const getImageUrl = (path: string | null | undefined, size: 'w500' | 'original' = 'w500'): string => {
  if (!path) {
    return `https://placehold.co/500x750/1e293b/a8a8a8?text=No+Image`;
  }
...
const Rating = ({ value, size = 16, showText = false }: { value: number; size?: number; showText?: boolean }) => {
  const normalizedValue = Math.round(value / 2);
  ...
  return (
    <div className="flex items-center gap-1 text-sm font-medium">
      ...
      {showText && <span className="text-sm text-white">{value.toFixed(1)}</span>}
      {!showText && <span className="text-xs text-gray-400">{value.toFixed(1)}</span>}
    </div>
  );
};

const MovieCard = ({ movie, onClick }: { movie: Movie; onClick: (m: Movie) => void }) => {
  const posterUrl = getImageUrl(movie.poster_path, 'w500');
  const releaseYear = movie.release_date ? movie.release_date.split('-')[0] : 'N/A';

  return (
    <div
      className="bg-dark-card rounded-xl overflow-hidden shadow-lg cursor-pointer transform hover:scale-[1.03] transition-all duration-300 active:scale-[0.98] group"
      ...
      <div className="relative pt-[150%]">
        ...
        <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors flex items-end p-2.5">
          ...
          <div className="w-full">
            ...
            <div className="mt-1">
              <Rating value={movie.vote_average} size={12} />
            </div>
          </div>
        </div>
      </div>
...
const AuthView = ({ onLogin }: { onLogin: (user: User) => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log(`Conectando a: ${API_URL}${endpoint}`);
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
          ...
          <div className="flex gap-2 flex-wrap">
            {details?.genres.map(g => (
              <span key={g.id} className="text-xs text-primary bg-primary/10 border border-primary/30 px-2 py-0.5 rounded-full">
                {g.name}
              </span>
            ))}
          </div>
      ...
      <div className="px-6 -mt-4 relative z-20">
        <div className="bg-[#1f2937] rounded-2xl p-5 border border-white/5 shadow-xl mb-6">
          ...
          <p className="text-gray-400 text-sm leading-relaxed">
            {displayDetails.overview || "No hay sinopsis disponible."}
          </p>
        </div>
                ...
                <div key={member.id} className="flex-shrink-0 w-24 text-center">
                  <div className="w-24 h-24 rounded-full overflow-hidden mb-1.5 bg-neutral-800 border-2 border-white/10">
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = `https://placehold.co/100x100/374151/d1d5db?text=`;
                      }}
                    />
                  </div>
...
// 3. PROFILE VIEW
const ProfileView = ({ user, onUpdate, onLogout }: { user: User, onUpdate: (u: User) => void, onLogout: () => void }) => {
        ...
        <div className="relative group">
          ...
          {isEditing && (
            <button
              ...
              className="absolute bottom-0 right-0 bg-primary text-white p-2.5 rounded-full shadow-lg border-2 border-[#111827] hover:bg-primary/90 transition-colors"
            >
              <Camera size={18} />
            </button>
          )}
        ...
        {!isEditing ? (
          <div className="text-center mt-4">
            ...
            <button
              ...
              className="mt-4 px-6 py-2 bg-white/5 border border-white/10 rounded-full text-sm font-medium hover:bg-white/10 transition-colors"
            >
              Editar Perfil
            </button>
          </div>
          ...
          <div className="w-full mt-6 space-y-4 max-w-sm">
            <div className="space-y-1">
              <label className="text-xs text-gray-400 ml-1">Nombre de usuario</label>
              <input
                type="text"
                value={formData.username}
                onChange={e => setFormData({ ...formData, username: e.target.value })}
                className="w-full bg-[#1f2937] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary"
              />
            </div>
            <div className="space-y-1">
              <label className="text-xs text-gray-400 ml-1">Biografía</label>
              <textarea
                value={formData.bio}
                onChange={e => setFormData({ ...formData, bio: e.target.value })}
                className="w-full bg-[#1f2937] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary min-h-[100px]"
              />
            </div>
            <div className="flex gap-3 pt-2">
              ...
              <button
                onClick={handleSave}
                className="flex-1 py-3 rounded-xl bg-primary text-white font-medium shadow-lg shadow-primary/20 hover:bg-primary/90 transition-colors"
              >
                Guardar
              </button>
            </div>
...
// 4. HOME / DISCOVER VIEW
const HomeView = ({ onMovieSelect }: { onMovieSelect: (m: Movie) => void }) => {
  ...
  return (
    <div className="min-h-screen bg-[#111827] pb-24 text-white">
      ...
      {!searchTerm && !currentLoading && featured && (
        <div className="relative h-96 w-full mb-6 cursor-pointer" onClick={() => onMovieSelect(featured)}>
          <img
            ...
            className="w-full h-full object-cover"
            onError={(e) => {
              (e.target as HTMLImageElement).src = getImageUrl(featured.poster_path);
            }}
          />
      ...
      <div className="px-4">
        <h3 className="text-lg font-bold text-white mb-4 pl-1 border-l-4 border-primary leading-none">
          {searchTerm ? 'Resultados de búsqueda' : 'Populares ahora'}
        </h3>
        ...
        {displayMovies.length === 0 && !currentLoading && searchTerm && (
...
function App() {
  ...
  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('cineRedUser');
    localStorage.removeItem('authToken'); // Borramos también el token
    setActiveTab('home');
    setSelectedMovie(null);
  };

  if (typeof document !== 'undefined' && !document.querySelector('#tailwind-config-script')) {
    const script = document.createElement('div');
    script.innerHTML = `${tailwindConfig}<link href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap" rel="stylesheet">`;
  ...
  if (selectedMovie) {
    return <MovieDetailsView movie={selectedMovie} onBack={() => setSelectedMovie(null)} />;
  }
  ...
  return (
    <div className="bg-dark-bg min-h-screen text-white font-sans selection:bg-primary selection:text-white">
      ...
      {activeTab === 'home' && (
        <HomeView onMovieSelect={setSelectedMovie} />
      )}

      {activeTab === 'profile' && (
        <ProfileView user={user} onUpdate={handleUpdateProfile} onLogout={handleLogout} />
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-dark-bg/90 backdrop-blur-lg border-t border-white/10 px-6 py-3 flex justify-around items-center z-50 pb-4">
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 transition-colors ${activeTab === 'profile' ? 'text-primary' : 'text-gray-500 hover:text-gray-300'}`}
        >
          {user.avatarUrl ? (
            <img src={user.avatarUrl} className={`w-6 h-6 rounded-full border-2 object-cover ${activeTab === 'profile' ? 'border-primary' : 'border-gray-500'}`} alt="avatar" />
          ) : (
